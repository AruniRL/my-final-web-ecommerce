...
//check if the file size more than 5mb
        if($_FILES["image"]["size"] < 5098888){

        //check if file already exists
        if(file_exists($target_file)){
            $uploadOk = 0;
            $message .="sorry, file already exits.";
            ...
            if(move_uploaded_file($image_tmp, $target_file)){
                $update_image= mysqli_query($con, "update user_s set image='$image' where id='$_SESSION[user_id]'");
        ...
        }//end the file size more than 5mb
        else{
