...
<?php
  ...
  <html>
      <head>
          <title>User Sign In And Sign Up</title>
  ...
  <td colspan="4">
      ...
      <span> Already have an account? <a href="shop.php?action=login">Login Now</a></span>
  </td>
  <tr>
  ...
  if(isset($_POST['register'])){
    ...
    if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
        ...
        $check_exist = mysqli_query($con, "select * from user_s where email='$email'");
        ...
        if($email_count > 0){
            ...
            $run_insert = mysqli_query($con, "insert into user_s (ip_address, name, email, password, country, city, contact, user_address, image) values ('$ip', '$name', '$email', '$password' ,'$country',  '$contact', ' $city', '$address', '$image')");

            if($run_insert){
                $sel_user = mysqli_query($con, "select * from user_s where email='$email'");
