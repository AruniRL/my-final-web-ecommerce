...
<?php
$all_categories= mysqli_query($con, "select * from product_category order by cat_id DESC");
...
if(isset($_GET['delete_cat'])){
    $delete_cat = mysqli_query($con, "delete from product_category where cat_id='$_GET[delete_cat]'");

    if($delete_cat){
        echo"<script>alert('Product category has been deleted successfuly')</script>";
...
if(isset($_POST['deleteAll'])){
    ...
    foreach($remove as $key){
        $run_remove= mysqli_query($con, "delete from product_category where cat_id='$key'");
